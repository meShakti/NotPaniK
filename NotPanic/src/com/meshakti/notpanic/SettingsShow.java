...
public class SettingsShow extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		while(c.moveToNext()){
			...
			Toast.makeText(getApplicationContext(), "Applicaton Started", Toast.LENGTH_SHORT).show();
		}
		ArrayAdapter<String>cAdapter=new ArrayAdapter<String>(getApplicationContext(),R.layout.spintext,contactsList);
		contacts.setAdapter(cAdapter);
		getContactList();
		
		addableContacts.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {

			@Override
			public void onItemSelected(AdapterView<?> adp, View v,
					int option, long id) {
				String contact=addableContacts.getSelectedItem().toString();
				for(String item:contactsList){
					if(contact.equals("Add Contacts")){
						return;
					}
					if(contact.equals(item)){
						Toast.makeText(getApplicationContext(), "Contact is already in list", Toast.LENGTH_SHORT).show();
						return;
					}
					
					if(contacts.getAdapter().getCount()>=MAX_CONTACTS){
						Toast.makeText(getApplicationContext(), "Only 5 Contacts Allowed", Toast.LENGTH_SHORT).show();
						return;
					}
		...
		contacts.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> adp, View v,
					int option, long id) {
				if(!contacts.getSelectedItem().toString().equals("See Contacts")){
					
					contactsList.remove(option);
					ArrayAdapter<String>cAdapter=new ArrayAdapter<String>(getApplicationContext(),R.layout.spintext,contactsList);
					contacts.setAdapter(cAdapter);
 					Toast.makeText(getApplicationContext(), "Contact is removed", Toast.LENGTH_SHORT).show();
				}
		...
		saveSettings.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				//A potentially dangerous code to use it first delete all the data then inserts it back;
				...
				if(contacts.getAdapter().getCount()!=1){
					...
					for(String name:contactsList){
						if(!name.equals("See Contacts")){
							...
							if(ins<0){
								Toast.makeText(getApplicationContext(),"Insert Error Please Reset all settings", Toast.LENGTH_SHORT).show();
								return;
							}
						}
					...
					Toast.makeText(getApplicationContext(),"Empty List.Please insert some contacts", Toast.LENGTH_SHORT).show();				}
			}
	...
	private void getContactList() {
		//Experimental work
				...
				Cursor c=resolver.query(contacts,null, android.provider.ContactsContract.Contacts.HAS_PHONE_NUMBER, null, android.provider.ContactsContract.Contacts.DISPLAY_NAME);
				if(c!=null && c.moveToFirst()){
					int phoneNameIndex=c.getColumnIndex(android.provider.ContactsContract.Contacts.DISPLAY_NAME);
					do{
						
						String phoneName=c.getString(phoneNameIndex);
						addableContactsList.add(phoneName);
						
						String id = c.getString(c.getColumnIndex(android.provider.ContactsContract.Contacts._ID));
						Cursor phCur = resolver.query(android.provider.ContactsContract.CommonDataKinds.Phone.CONTENT_URI,
							        null,
						...
						phCur.moveToNext(); 
						String phoneNo =   phCur.getString(phCur.getColumnIndex(android.provider.ContactsContract.CommonDataKinds.Phone.NUMBER));
						contactsNumber.put(phoneName, phoneNo);	    
						
						
					}while(c.moveToNext());
					
					ArrayAdapter<String>aclAdapter=new ArrayAdapter<String>(getApplicationContext(),R.layout.spintext,addableContactsList);
